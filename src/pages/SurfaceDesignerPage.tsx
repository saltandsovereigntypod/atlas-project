...
import { ArrowLeft, Copy, Square, Trash2, Type } from 'lucide-react'
...
export default function SurfaceDesignerPage() {
  ...
  const load = async () => {
    ...
    setDatabase(db); setFields(f); setRecords(r); setLayout(l); setPreviewId((v) => v || recordId || r[0]?.id || '')
  ...
  const add = async (type: LayoutElement['type']) => {
    if (!layout) return
    ...
    const item = await createLayoutElement(layout.id, {
      ...
      width: type === 'shape' ? 220 : 280,
      height: type === 'shape' ? 120 : 64,
      rotation: 0,
      ...
      props: type === 'shape'
        ...
        : type === 'text'
          ...
          : { source: defaultField ? 'field' : 'title', fontSize: 24, fontWeight: 600, color: '#211f1b', align: 'left', fontFamily: 'inherit', opacity: 1, imageMode: false },
    })
    setElements((current) => [...current, item]); setSelectedId(item.id)
  }
  ...
  const duplicate = async () => {
    ...
    setElements((current) => [...current, created]); setSelectedId(created.id)
  }

  const remove = async () => {
    if (!selected) return
    await deleteLayoutElement(selected.id)
    setElements((current) => current.filter((item) => item.id !== selected.id)); setSelectedId(null)
  }
  ...
  return (
    <div className="surface-designer">
      ...
      <div className="surface-designer-body">
        <aside className="surface-tools">
          <p className="panel-label">ADD</p>
          <button onClick={() => add('text')}><Type size={16} /> Text</button>
          <button onClick={() => add('field')}>+ Property</button>
          <button onClick={() => add('shape')}><Square size={16} /> Shape</button>
          ...
          <div className="layer-list">{[...sorted].reverse().map((item) => <button className={selectedId === item.id ? 'active' : ''} key={item.id} onClick={() => setSelectedId(item.id)}>{item.type === 'field' ? fields.find((f) => f.id === item.binding_field_id)?.name || 'Property' : item.type === 'text' ? String(item.props.text || 'Text').slice(0, 20) : 'Shape'}</button>)}</div>
        </aside>
        <main className="surface-canvas-stage">
          <div className="design-canvas show-grid" style={{ width: layout.canvas_width, height: layout.canvas_height, backgroundColor: layout.background }}>
            {sorted.map((item) => {
              ...
              return <div key={item.id} className={`canvas-element text-element ${selectedNow ? 'selected' : ''}`} style={{ ...common, fontSize: Number(item.props.fontSize || 24), fontWeight: Number(item.props.fontWeight || 400), color: String(item.props.color || '#211f1b'), textAlign: String(item.props.align || 'left') as 'left' | 'center' | 'right', fontFamily: String(item.props.fontFamily || 'inherit'), opacity: Number(item.props.opacity ?? 1) }} onPointerDown={(e) => dragStart(e, item)} onPointerMove={pointerMove} onPointerUp={pointerUp}>{imageMode && content ? <img src={content} alt="" draggable={false} /> : content}{selectedNow && <div className="resize-handle" onPointerDown={(e) => resizeStart(e, item)} onPointerMove={pointerMove} onPointerUp={pointerUp} />}</div>
            })}
          </div>
        </main>
        <aside className="surface-inspector">
          ...
          {!selected ? <p className="panel-note">Select a layer to change what it shows and how it looks.</p> : <>
            ...
            {selected.type === 'field' && <label>Property<select value={selected.props.source === 'title' ? '__title' : selected.binding_field_id || ''} onChange={(e) => e.target.value === '__title' ? patch({ binding_field_id: null, props: { ...selected.props, source: 'title' } }) : patch({ binding_field_id: e.target.value, props: { ...selected.props, source: 'field' } })}><option value="__title">Record title</option>{fields.filter((f) => !(f.position === 0 && f.name === 'Name')).map((f) => <option value={f.id} key={f.id}>{f.name}</option>)}</select></label>}
