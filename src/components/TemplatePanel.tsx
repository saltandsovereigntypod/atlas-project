...
export default function TemplatePanel({page,blocks,databases,database}:Props){
  ...
  const apply=async(templateId:string,mode:'replace'|'add')=>{
    ...
    setBusy(`${templateId}:${mode}`);setError('')
    try{
      if(mode==='replace'){
  ...
  return <div className="atlas-template-panel">
    <div className="atlas-template-intro"><Sparkles/><div><strong>Templates</strong><span>Designed starting points made from normal Atlas elements. Apply one, then change absolutely everything.</span></div></div>
    ...
    <div className="atlas-template-list">{items.map(template=><article key={template.id} className="atlas-template-card">
      ...
      <div className="atlas-template-actions"><button disabled={Boolean(busy)} onClick={()=>void apply(template.id,'replace')}><LayoutTemplate/>Replace design</button><button disabled={Boolean(busy)} onClick={()=>void apply(template.id,'add')}><Plus/>Add to page</button></div>
    </article>)}</div>
