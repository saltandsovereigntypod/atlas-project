...
import { Database as DatabaseIcon, Layers3, LayoutTemplate, Link2, Palette, Plus, Search, Sparkles, WandSparkles } from 'lucide-react'
...
const recommendedKit = (category: TemplateCategory) => category === 'Budget' ? 'budget-system' : category === 'Books' ? 'reading-system' : category === 'Podcast' ? 'podcast-system' : category === 'Travel' ? 'travel-system' : category === 'Witchy' ? 'witchy-journal-system' : ''
const primaryCollection = (category: TemplateCategory) => category === 'Budget' ? 'Transactions' : category === 'Books' ? 'Books' : category === 'Podcast' ? 'Episodes' : category === 'Travel' ? 'Trips' : category === 'Witchy' ? 'Grimoire' : ''
...
export default function TemplatePanel({ page, blocks, databases, database }: Props) {
  ...
  const createSource = async () => {
    ...
    try {
      ...
      setPreferredDb(created.id)
      await refresh()
      setNotice(`${created.name} is ready. You can add entries below without opening a table.`)
      setSourceOpen(false)
  ...
  const ensureKit = async (kitId: string) => {
    ...
    if (!kit) return { firstId:'', ids:new Map<string,string>(), createdNames:[] as string[] }
    const latest = new Map(databases.map(item => [item.name.toLowerCase(), item]))
    const ids = new Map<string,string>()
    const createdNames: string[] = []
    let firstId = ''
    for (const collection of kit.collections) {
      const found = latest.get(collection.name.toLowerCase())
      if (found) {
        ids.set(collection.name, found.id)
        if (!firstId) firstId = found.id
        continue
      }
      const created = await createDatabase(page.workspace_id, collection.name)
      ids.set(collection.name, created.id)
    ...
    return { firstId, ids, createdNames }
  }
  ...
  const createKit = async (kitId: string) => {
    ...
    try {
      const result = await ensureKit(kitId)
      if (result.firstId) setPreferredDb(result.firstId)
      await refresh()
      setNotice(result.createdNames.length ? `${kit.name} created: ${result.createdNames.join(', ')}.` : `Your ${kit.name} collections already exist. Atlas left them alone.`)
    } catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(null) }
  }
  ...
  const applyTemplate = async (templateId: string, applyMode: 'replace' | 'add', sourceId = databaseId) => {
    ...
    try {
      ...
      await refresh()
      setNotice(`${template.name} applied${sourceId ? ' and connected to your selected data.' : ''} Keep editing, moving, or restyling anything.`)
    } catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(null) }
  }

  const setupAndUse = async (templateId: string) => {
    const template = PAGE_TEMPLATES.find(item => item.id === templateId)
    if (!template) return
    const kitId = recommendedKit(template.category)
    if (!kitId) { await applyTemplate(templateId, 'replace'); return }
    if (!window.confirm(`Set up the recommended data system and replace this page’s visual design with “${template.name}”? Your existing data will not be deleted.`)) return
    setBusy(`setup:${templateId}`); clearMessages()
    try {
      const result = await ensureKit(kitId)
      const wanted = primaryCollection(template.category)
      const sourceId = result.ids.get(wanted) || result.firstId
      if (sourceId) setPreferredDb(sourceId)
      if (blocks.length) await Promise.all(blocks.map(block => deletePageBlock(block.id)))
      await updatePage(page.id, { settings:{ ...page.settings, ...template.settings } })
      for (let i = 0; i < template.blocks.length; i++) {
        const seed = template.blocks[i]
        await createPageBlock(page.id, seed.type, i, resolveTemplateConfig(seed.config, sourceId))
      }
      await refresh()
      setNotice(`${template.name} is live. ${result.createdNames.length ? `Atlas created ${result.createdNames.join(', ')}. ` : 'Your existing collections were reused. '}The page is connected and ready for real entries.`)
    } catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(null) }
  }

  const addSection = async (sectionId: string) => {
    const section = SECTION_PRESETS.find(item => item.id === sectionId)
    if (!section) return
    ...
    try {
      ...
      for (let i = 0; i < section.blocks.length; i++) {
        const seed = section.blocks[i]
        await createPageBlock(page.id, seed.type, blocks.length + i, resolveCreativeConfig(seed.config, databaseId, 55, yOffset, zOffset))
      }
      ...
      await refresh()
      setNotice(`${section.name} added below your current design. Every piece is independent and editable.`)
    } catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(null) }
  }
  ...
  const applyStyle = async (styleId: string) => {
    ...
    try {
      ...
      await refresh()
      setNotice(`${pack.name} applied without changing your layout or data bindings.`)
    } catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(null) }
  }
  ...
  const bindExisting = async () => {
    ...
    try {
      ...
      for (const block of blocks) {
        if (block.type === 'database_view' || block.type === 'metric' || (block.type === 'widget' && needsDatabase(String(block.config.widgetType || '')))) {
          await updatePageBlock(block.id, { config:{ ...block.config, databaseId } }); count++
        }
      }
      await refresh()
      setNotice(count ? `Connected ${count} data-aware element${count === 1 ? '' : 's'} to ${selectedDatabase?.name || 'that collection'}.` : 'There are no data-aware elements on this page yet.')
    } catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(null) }
  }

  return <div className="atlas-template-panel">
    <div className="atlas-template-intro"><Sparkles/><div><strong>Build something gorgeous</strong><span>Pages, drop-in sections, visual styles, and friendly data systems. Everything stays editable.</span></div></div>
    ...
    {mode === 'templates' && <><DataBindingBar databases={databases} value={databaseId} onChange={setPreferredDb}/><div className="atlas-template-categories">{categories.map(c => <button key={c} className={category === c ? 'active' : ''} onClick={() => setCategory(c)}>{c}</button>)}</div><div className="atlas-template-list">{templateItems.map(template => { const kitId = recommendedKit(template.category); return <article key={template.id} className="atlas-template-card"><div className="atlas-template-preview"><span>{template.icon}</span><small>{template.category}</small></div><div className="atlas-template-copy"><strong>{template.name}</strong><p>{template.description}</p>{kitId && <span className="atlas-template-system-badge"><Sparkles/>Can set up its data automatically</span>}</div>{kitId && <button className="atlas-template-magic" disabled={Boolean(busy)} onClick={() => void setupAndUse(template.id)}><WandSparkles/>{busy === `setup:${template.id}` ? 'Building…' : 'Set up & use'}</button>}<div className="atlas-template-actions"><button disabled={Boolean(busy)} onClick={() => void applyTemplate(template.id, 'replace')}><LayoutTemplate/>{busy === `${template.id}:replace` ? 'Applying…' : 'Replace design'}</button><button disabled={Boolean(busy)} onClick={() => void applyTemplate(template.id, 'add')}><Plus/>{busy === `${template.id}:add` ? 'Adding…' : 'Add to page'}</button></div></article> })}</div></>}
    ...
    {mode === 'data' && <><div className="atlas-source-starter"><div className="atlas-source-starter-copy"><DatabaseIcon/><div><strong>What should this page track?</strong><span>Create one collection without building a spreadsheet first.</span></div></div><button className="atlas-source-create" onClick={() => setSourceOpen(value => !value)}><Plus/>{sourceOpen ? 'Close setup' : 'Create one collection'}</button>{sourceOpen && <div className="atlas-source-wizard"><div className="atlas-source-preset-grid">{SOURCE_PRESETS.map(preset => <button key={preset.id} className={sourcePreset === preset.id ? 'active' : ''} onClick={() => choosePreset(preset.id)}><span>{preset.icon}</span><strong>{preset.name}</strong><small>{preset.description}</small></button>)}</div><label className="atlas-source-name">What do you want to call it?<input value={sourceName} onChange={e => setSourceName(e.target.value)} placeholder="Transactions"/></label><div className="atlas-source-summary"><strong>Atlas will handle the structure.</strong><span>{SOURCE_PRESETS.find(item => item.id === sourcePreset)?.fields.length ? `We’ll create ${SOURCE_PRESETS.find(item => item.id === sourcePreset)?.fields.length} useful properties. You can change or delete any of them later.` : 'We’ll create a clean collection with a Name field.'}</span></div><button className="atlas-source-finish" disabled={busy === 'source' || !sourceName.trim()} onClick={() => void createSource()}>{busy === 'source' ? 'Creating…' : 'Create & keep designing'}</button></div>}</div><div className="atlas-bind-card"><div><Link2/><span><strong>Connect this page</strong><small>Choose a collection, then connect all compatible widgets and views in one click.</small></span></div><DataBindingBar databases={databases} value={databaseId} onChange={setPreferredDb}/><button disabled={!databaseId || Boolean(busy)} onClick={() => void bindExisting()}>{busy === 'bind' ? 'Connecting…' : 'Connect data-aware elements'}</button></div><div className="atlas-friendly-entry"><div className="atlas-kit-heading"><Sparkles/><div><strong>Add real information</strong><span>This is the everyday view. Atlas builds the form from your collection so you do not have to work in a table.</span></div></div><FriendlyRecordForm database={selectedDatabase} onSaved={refresh}/></div><div className="atlas-kit-heading"><Sparkles/><div><strong>Or build a whole system</strong><span>Atlas can create the related collections that make a template actually useful.</span></div></div><div className="atlas-data-kit-list">{kitItems.map(kit => <article key={kit.id} className="atlas-data-kit-card"><div><span>{kit.icon}</span><div><strong>{kit.name}</strong><p>{kit.description}</p></div></div><small>{kit.collections.map(collection => collection.name).join(' · ')}</small><button disabled={Boolean(busy)} onClick={() => void createKit(kit.id)}><Plus/>{busy === `kit:${kit.id}` ? 'Building…' : 'Create system'}</button></article>)}</div></>}
  </div>
}

function DataBindingBar({ databases, value, onChange }: { databases: Database[]; value: string; onChange: (id: string) => void }) {
  return <div className="atlas-binding-bar"><Link2/><div><strong>Template data</strong><small>{databases.length ? 'Data-aware pieces added next will use this collection.' : 'No collections yet. Create one from Data first.'}</small></div><select value={value} disabled={!databases.length} onChange={e => onChange(e.target.value)}><option value="">No data</option>{databases.map(db => <option key={db.id} value={db.id}>{db.name}</option>)}</select></div>
}
