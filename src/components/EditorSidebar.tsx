...
import { AlignCenter, AlignLeft, AlignRight, ArrowDown, ArrowUp, BarChart3, Clock3, Copy, Database, Eye, EyeOff, Heading, Image, Layers, Lock, Minus, MousePointer2, Palette, Plus, Quote, SlidersHorizontal, Square, Trash2, Type, Unlock, Upload } from 'lucide-react'
...
function AddPanel({ onAdd, canProperty }: { onAdd: (type: PageBlockType) => void; canProperty: boolean }) {
  return <>
    ...
    <PanelTitle>Widgets</PanelTitle><div className="atlas-add-grid"><Add icon={<Clock3 />} label="Widget library" onClick={() => onAdd('widget')} /></div><p className="atlas-helper">Add a widget, then choose Clock, To-do, Calendar, Countdown, Habit, Database tools, and more in Design.</p>
...
function layerName(layer: PageBlock) {
  ...
  if (layer.type === 'widget') return String(layer.config.widgetType || 'Widget').replaceAll('_',' ')
