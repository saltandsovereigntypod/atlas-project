...
import { Bold, Italic, Quote, Search, Sparkles, Underline } from 'lucide-react'
...
export default function WidgetSettings({ config, databases, save }: Props) {
  ...
  return <div className="widget-settings">
    ...
    {type === 'digital_clock' && <><label className="atlas-edit-control"><span>Time format</span><select value={String(config.timeFormat || '12h')} onChange={event => save({ timeFormat: event.target.value })}><option value="12h">12 hour</option><option value="24h">24 hour</option></select></label><FontChoice label="Time font" value={String(config.timeFontFamily||'')} onChange={value=>save({timeFontFamily:value})}/><FontChoice label="Date font" value={String(config.dateFontFamily||'')} onChange={value=>save({dateFontFamily:value})}/><Check label="Show seconds" checked={config.showSeconds !== false} onChange={value => save({ showSeconds: value })}/><Check label="Show date" checked={config.showDate !== false} onChange={value => save({ showDate: value })}/></>}
    ...
    {(type === 'daily_focus' || type === 'sticky_note') && <><RichTextControls/><label className="atlas-edit-control"><span>Text</span><textarea value={stripHtml(String(config.text || ''))} onChange={event => save({ text:event.target.value })}/></label><p className="atlas-helper">Select text inside the widget, then use these controls for rich formatting. Press Enter on the widget to log an entry underneath.</p></>}
...
const widgetFontChoices=['','Arial, sans-serif','Georgia, serif','Times New Roman, serif','Helvetica, Arial, sans-serif','Verdana, sans-serif','Courier New, monospace','Impact, sans-serif','system-ui, sans-serif']
function FontChoice({label,value,onChange}:{label:string;value:string;onChange:(value:string)=>void}){return <label className="atlas-edit-control"><span>{label}</span><select value={widgetFontChoices.includes(value)?value:''} onChange={event=>onChange(event.target.value)}><option value="">Use widget font</option>{widgetFontChoices.filter(Boolean).map(font=><option key={font} value={font}>{font.split(',')[0]}</option>)}</select></label>}
...
function RichTextControls(){const apply=(command:string,value?:string)=>document.execCommand(command,false,value);return <div className="widget-inspector-rich-controls"><button type="button" title="Bold" onMouseDown={event=>{event.preventDefault();apply('bold')}}><Bold/></button><button type="button" title="Italic" onMouseDown={event=>{event.preventDefault();apply('italic')}}><Italic/></button><button type="button" title="Underline" onMouseDown={event=>{event.preventDefault();apply('underline')}}><Underline/></button><button type="button" title="Block quote" onMouseDown={event=>{event.preventDefault();apply('formatBlock','blockquote')}}><Quote/></button></div>}
function stripHtml(value:string){if(typeof document==='undefined')return value.replace(/<[^>]*>/g,'');const div=document.createElement('div');div.innerHTML=value;return div.textContent||''}
